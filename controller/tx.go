...
func (c *Controller) GetProposalBlockFromMempool() *CachedProposal {
        // the cached proposal may not have been populated yet (e.g. before the first CheckMempool() run),
        // in which case the atomic.Value is still its zero value and the type assertion would panic
        cached, ok := c.Mempool.cachedProposal.Load().(*CachedProposal)
        if !ok {
                return nil
        }
        return cached
