...
func (c *Controller) ValidateProposal(rcBuildHeight uint64, qc *lib.QuorumCertificate, evidence *bft.ByzantineEvidence) (blockResult *lib.BlockResult, err lib.ErrorI) {
	...
	blockResult, err = c.ApplyAndValidateBlock(block, c.LastValidatorSet[c.ChainHeight()][c.Config.ChainId], rcBuildHeight, false)
	if err != nil {
		// exit with error
		return
	}
...
func (c *Controller) CommitCertificate(qc *lib.QuorumCertificate, block *lib.Block, blockResult *lib.BlockResult, ts uint64) (err lib.ErrorI) {
	...
	if blockResult == nil {
		...
		blockResult, err = c.ApplyAndValidateBlock(block, c.LastValidatorSet[c.ChainHeight()][c.Config.ChainId], qc.Header.RootHeight, true)
		if err != nil {
			// exit with error
			return
		}
	...
	c.FSM, err = fsm.New(c.Config, storeI, c.Plugin, c.Metrics, c.RCManager, c.log)
...
func (c *Controller) CommitCertificateParallel(qc *lib.QuorumCertificate, block *lib.Block, blockResult *lib.BlockResult, ts uint64) (err lib.ErrorI) {
	...
	eg.Go(func() error {
		...
		c.Mempool.FSM, err = fsm.New(c.Config, memPoolStore, c.Plugin, c.Metrics, c.RCManager, c.log)
		if err != nil {
			// exit with error
			return err
		}
...
func (c *Controller) ApplyAndValidateBlock(block *lib.Block, lastValidatorSet *lib.ValidatorSet, rcBuildHeight uint64, commit bool) (b *lib.BlockResult, err lib.ErrorI) {
	...
	// apply the block against the state machine
	compare, txResults, _, failed, err := c.FSM.ApplyBlock(context.Background(), block, lastValidatorSet, rcBuildHeight, false)
	if err != nil {
		// exit with error
		return
	}
