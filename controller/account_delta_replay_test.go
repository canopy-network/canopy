...
func newBareReplayController(t *testing.T, versions uint64, blockAtLastVersion *lib.BlockResult) *Controller {
	...
	sm.Reset()
	return &Controller{FSM: sm, accountDeltaCache: newAccountDeltaCache(0), log: log}
}
...
func newAccountDeltaChainController(t *testing.T, finalQCMutate func(*lib.QuorumCertificate)) *Controller {
	...
	return &Controller{FSM: sm, Config: sm.Config, accountDeltaCache: newAccountDeltaCache(0), log: log}
}
...
// full production path (cold tip cache) and checks the classified delta against the
// committed state at versions 5 and 6
func TestGetAccountDelta_ReplayHappyPath(t *testing.T) {
	...
	// the on-demand replay is a pure read: it must not retro-populate the tip cache (only
	// a successful live commit does that)
	_, ok := c.accountDeltaCache.get(5)
	require.False(t, ok, "the replay path must not populate the tip cache")
	c.accountDeltaCache.mu.RLock()
	require.Empty(t, c.accountDeltaCache.entries, "the replay path must not cache anything at all")
...
func TestGetAccountDelta_HeightTwoBoundarySkipsQC(t *testing.T) {
	...
	setFSMHeight(t, sm, 2)
	sm.Reset()
	c := &Controller{FSM: sm, accountDeltaCache: newAccountDeltaCache(0), log: log}
