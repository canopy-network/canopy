...
var (
	// predefine big int constants for memory efficiency
...
// The important equations are -> (ax^2 + bxy +cy^2) and (b^2 - 4ac = Discriminant)
// ClassGroup provides operations for class groups of quadratic imaginary fields, which form the basis for constructing Verifiable Delay Functions
// These groups enable efficient sequential computations with fast verification, leveraging their cryptographic hardness properties
// Unlike RSA-based VDFs, class groups don’t require a trusted setup
type ClassGroup struct {
	a *big.Int // a value in ax^2 + bxy + cy^2
	b *big.Int // b value in ax^2 + bxy + cy^2
	c *big.Int // c value in ax^2 + bxy + cy^2
	d *big.Int // discriminant value
}

// NewClassGroup() is a constructor for the class group object
func NewClassGroup(a, b, c *big.Int) *ClassGroup { return &ClassGroup{a: a, b: b, c: c} }

// CloneClassGroup() creates a shallow copy of the class group
func CloneClassGroup(cg *ClassGroup) *ClassGroup { return &ClassGroup{a: cg.a, b: cg.b, c: cg.c} }
...
// Reduced() transforms a given class group element into a more compact and canonical representation of the group element
// reduction ensures that (a <= c) && (b >= 0 when a == c)
...
func (group *ClassGroup) Reduced() *ClassGroup {
	...
	for (a.Cmp(c) == 1) || ((a.Cmp(c) == 0) && (b.Sign() == -1)) {
		...
		tempA.Set(a)
		tempB.Set(b)
		// calculate the integer value of s, which minimizes b by aligning it closer to 0.
		// s = floor((c + b) / (2 * c))
		tempS.Add(c, b)
		tempS = floorDivision(tempS, tempC2.Add(c, c))
		// a = c
		a.Set(c)
		// b = -2b * s * c
		b.Neg(b)
		tempX.Mul(bigTwo, tempS)
		tempX.Mul(tempX, c)
		b.Add(b, tempX)
		// c = c * s^2 - b * s + a
		c.Mul(c, tempS)
		c.Mul(c, tempS)
		tempB.Mul(tempB, tempS)
		c.Sub(c, tempB)
		c.Add(c, tempA)
	}
...
func NewDiscriminant(seed []byte) *big.Int {
	...
	// ensure 2048 bits
	n.SetBit(n, 2047, 1)
	// ensure divisibility by m
	n.Sub(n, temp.Mod(n, bigM))
	// use the last two bytes as the index input for the residue
	// without it - n might fail to meet the modular constraint
	residueIndex := int(binary.BigEndian.Uint16(entropy[len(entropy)-2:])) % len(Residues)
	residue := Residues[residueIndex]
	// add the residue to the value
	...
	for {
		...
		// this effectively shifts the search to the next segment of size 2^16
		n = temp.Add(n, temp.Mul(bigM, big65536))
	}
}
...
func floorDivision(x, y *big.Int) *big.Int {
	...
	// 1. The remainder (r) is positive, and the divisor (y) is negative
	// 2. The remainder (r) is negative, and the divisor (y) is positive
	if (r.Sign() == 1 && y.Sign() == -1) || (r.Sign() == -1 && y.Sign() == 1) {
		// subtract 1 from the quotient to round down, achieving floor division.
		q.Sub(q, bigOne)
	}
	return q
}

// solveMod solves the modular equation a * x ≡ b (mod m)
// - s: A particular solution (x) to the equation
...
func solveMod(a, b, m *big.Int) (solution, step *big.Int, solvable bool) {
	...
	remainder := solution.Mod(b, gcd)
	if remainder.Cmp(bigZero) != 0 {
		return nil, nil, false // No solution exists
	}
	// calculate the solution
	solution.Div(b, gcd)      // b / gcd
