...
func (l *SimpleLimiter) NewRequest(requester string) (requesterBlock, totalBlock bool) {
	...
	if l.totalRequests >= l.maxRequests {
		l.log.Infof("Request from %s was total blocked in topic %s", requester, l.topic)
	...
	if count := l.requests[requester]; count >= l.maxPerRequester {
		l.log.Infof("Request from %s was blocked without total block in topic %s", requester, l.topic)
	...
	l.totalRequests++
	l.log.Infof("Request from %s was not blocked in topic %s", requester, l.topic)
	// exit
	return
}
