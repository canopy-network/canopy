...
func NewLimiter(maxPerRequester, maxRequests, resetWindowS int, log LoggerI) *SimpleLimiter {
...
func (l *SimpleLimiter) NewRequest(requester string) (requesterBlock, totalBlock bool) {
	...
	if l.totalRequests >= l.maxRequests {
		l.log.Infof("Request from %s was total blocked", requester)
	...
	if count := l.requests[requester]; count >= l.maxPerRequester {
		l.log.Infof("Request from %s was blocked without total block", requester)
	...
	l.totalRequests++
	l.log.Infof("Request from %s was not blocked", requester)
	// exit
	return
}
