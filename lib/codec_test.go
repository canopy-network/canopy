...
func TestTransactionJSONUsesHexForPluginBytes(t *testing.T) {
	...
	const pluginMessageType = "plugin-signature"
	desc := (&Signature{}).ProtoReflect().Descriptor()
	globalPluginSchemaRegistry.byCommonMessageName[pluginMessageType] = desc

	expected := bytes.Repeat([]byte{0xab}, 20)
	jsonBytes := []byte(`{"type":"` + pluginMessageType + `","msg":{"publicKey":"` + BytesToString(expected) + `"}}`)
	transaction := new(Transaction)
