# CPLQ Tokenomics v1.2 — Full Detail

Source: `docs/canoLiq_Tokenomics_v1.2.pdf` (9 pages, May 2025).
Subtitle: "CPLQ Token Design & Protocol Economics". Total Supply: **100,000,000
CPLQ — Fixed Supply, No Additional Minting.**

## 1. Token Overview & Design Philosophy

CPLQ is the governance and value-capture token of canoLiq. Deliberately minimal
...
| Secondary Token | cCNPY (yield-bearing liquid staking receipt; distinct from CPLQ) |
...
### 2.1 Validators & Infrastructure — 22M CPLQ
...
- **12-month cliff rationale:** validators run production infra from day one,
  but CPLQ should not unlock until the protocol survives its first full year of
...
### 2.2 Community & Airdrops — 20M CPLQ
...
### 2.3 DAO Treasury — 15M CPLQ
The protocol's strategic reserve. Governed entirely by CPLQ holders via on-chain
proposals with timelock enforcement.
- **Time-locked multisig:** 3-of-5 signer multisig with 48-hour timelock for
  standard spends, 7-day for large spends (>1M CPLQ equivalent).
...
### 2.6 Strategic Partners — 10M CPLQ
...
- Partner criteria and allocations subject to DAO approval above 500K CPLQ per
  partner.

### 2.7 Developer Grants & Ecosystem — 6M CPLQ
...
protocol revenue and drives all value accrual to CPLQ holders.
...
| CPLQ Buyback | 15% | 1.8% | Open-market CPLQ purchase; **default action is Burn**. DAO may vote quarterly to distribute to locked CPLQ stakers instead. |

### 3.3 Fee Rate Governance
12% default rate is governance-controlled. CPLQ holders may vote to adjust within
protocol-defined bounds of **5% to 20%**. Changes require a standard governance
proposal with a 48-hour timelock.

## 4. CPLQ Value Accrual Mechanics

### 4.1 Buyback Engine
15% of all protocol fees buy CPLQ on the open market. Default action for
purchased CPLQ is to **burn** it (permanently reduces circulating supply).
Quarterly, CPLQ holders may vote to redirect buyback proceeds to locked CPLQ
...
| TVL (CNPY) | Annual Fees to Protocol | Annual CPLQ Buyback (15%) |
|---|---|---|
| $5M | $48,000 | $7,200 |
| $25M | $240,000 | $36,000 |
...
protocol's own token, flooding the market with freshly minted CPLQ before the
...
not newly printed CPLQ. The 15M CPLQ liquidity allocation then stretches over
24+ months instead of front-loading the first few months.

> **Why this matters for CPLQ holders:** every CPLQ NOT emitted in months 1–6 is
> CPLQ not sold by mercenary farmers. Subsidy substitution is the primary
> mechanism by which canoLiq's liquidity incentive budget preserves CPLQ value —
...
block one with zero additional CPLQ required.
...
| Scenario | Effective CPLQ Emission (Liquidity Bucket, 24 mo.) | Impact on CPLQ Supply |
|---|---|---|
| No subsidy approved | 15M CPLQ (full allocation, front-loaded) | Heavy early sell pressure; runway exhausted by month 18 |
| 6-month CNPY subsidy | ~8–10M CPLQ over 24 months | ~30–45% reduction in early emission; runway extended |
| 12-month CNPY subsidy | ~5–8M CPLQ over 24 months | ~50–60% reduction; pool active well beyond year 2 |
...
CPLQ is the sole governance token. All protocol parameter changes, treasury
actions, and strategic decisions require CPLQ holder votes.
...
- **Slashing risk:** canoLiq's CNPY stake is exposed to committee slashing;
  cCNPY and CPLQ holders bear this proportionally. Insurance fund of 5% of DAO
...
- **CPLQ liquidity risk:** early CPLQ markets may be illiquid; large sellers may
...
- **Governance centralization:** a single entity accumulating >33% of circulating
  CPLQ can influence most votes. Quadratic / conviction voting may be explored
  post-launch.
...
change prior to and following mainnet launch. CPLQ and cCNPY involve significant
risk.
