...
func NewStoreWithDB(config lib.Config, db *pebble.DB, metrics *lib.Metrics, log lib.LoggerI) (*Store, lib.ErrorI) {
	...
	return &Store{
		...
		Indexer:    &Indexer{NewTxn(hssStore, hssStore, indexerPrefix, false, false, false, nextVersion), config, newValidatorTotalsCache(), newCommitteeCache(), metrics},
		metrics:    metrics,
		config:     config,
		mu:         &sync.Mutex{},
...
func (s *Store) NewReadOnly(queryVersion uint64) (lib.StoreI, lib.ErrorI) {
	...
	return &Store{
		...
		Indexer:    &Indexer{NewTxn(hssReader, nil, indexerPrefix, false, false, false), s.config, s.Indexer.totals, s.Indexer.committees, s.metrics},
		metrics:    s.metrics,
		mu:         &sync.Mutex{},
		compaction: atomic.Bool{},
		backup:     atomic.Bool{},
...
func (s *Store) Copy() (lib.StoreI, lib.ErrorI) {
	...
	return &Store{
		...
		Indexer:    &Indexer{s.Indexer.db.Copy(reader, reader), s.config, s.Indexer.totals, s.Indexer.committees, s.metrics},
		metrics:    s.metrics,
		mu:         &sync.Mutex{},
		compaction: atomic.Bool{},
		backup:     atomic.Bool{},
...
func (s *Store) NewTxn() lib.StoreI {
	...
	return &Store{
		...
		Indexer: &Indexer{NewTxn(s.Indexer.db, s.Indexer.db, nil, false, true, false, nextVersion), s.config, s.Indexer.totals, s.Indexer.committees, s.metrics},
		metrics: s.metrics,
		mu:      s.mu,
		isTxn:   true,
	}
