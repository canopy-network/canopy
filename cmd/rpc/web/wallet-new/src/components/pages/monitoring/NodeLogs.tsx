...
export default function NodeLogs({
  ...
  const MAX_LOGS = 1000; // Límite máximo de logs

  // Limitar logs para evitar acumulación excesiva
  const limitedLogs = useMemo(() => {
    return logs.slice(-MAX_LOGS);
  }, [logs]);

  // Memoizar el formateo de logs para evitar re-cálculos
  const formatLogLine = useCallback((line: string) => {
    // Optimización: usar un solo regex compilado
    const patterns = [
  ...
  // Memoizar logs visibles para evitar re-renderizados
  ...
  // Auto-scroll al final cuando lleguen nuevos logs
  useEffect(() => {
  ...
  // Memoizar el componente de log individual
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          <h2 className="text-text-primary text-lg font-bold">
            {getText('ui.nodeLogs.title', 'Node Logs')}
          </h2>
