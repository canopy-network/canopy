...
export default function NetworkPeers({ networkPeers }: NetworkPeersProps): JSX.Element {
  ...
  return (
    <div className="bg-bg-secondary rounded-xl border border-bg-accent p-6">
      <h2 className="text-text-primary text-lg font-bold mb-4">{getText('ui.networkPeers.title', 'Network Peers')}</h2>
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          <div className="text-text-muted text-xs">{getText('ui.networkPeers.totalPeers', 'Total Peers')}</div>
          <div className="text-text-accent text-2xl font-bold">{networkPeers.totalPeers}</div>
        </div>
        <div>
          <div className="text-text-muted text-xs">{getText('ui.networkPeers.connections', 'Connections')}</div>
      ...
      <div className="space-y-2">
        <div className="md:w-4/12 w-full">
          <div className="text-text-muted text-xs">{getText('ui.networkPeers.peerId', 'Peer ID')}</div>
          <div className="text-text-primary font-mono text-sm truncate">{networkPeers.peerId}</div>
        </div>
        <div>
          <div className="text-text-muted text-xs">{getText('ui.networkPeers.networkAddress', 'Network Address')}</div>
