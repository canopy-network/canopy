...
export default function PastProposals({ proposals }: PastProposalsProps): JSX.Element {
  const { getText } = useManifest();
  const title = getText('ui.governance.sections.pastProposals', 'Past Proposals');

  const entries = Object.entries(proposals || {}).slice(0, 5);
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {entries.map(([hash, data], index) => {
            ...
            const proposalTitle = index === 0 ? 'Enable Cross-Chain Bridges' : 'Increase Staking Rewards';
            const proposalDescription = index === 0 
              ...
              <tr key={hash} className="border-t border-bg-accent">
                <td className="py-3">
                  <div className="flex items-center gap-2">
                    <span className="text-white">{proposalId}:</span>
                    <span className="text-white font-medium">{proposalTitle}</span>
                  </div>
                  <div className="text-text-muted text-xs mt-1">{proposalDescription}</div>
                </td>
                <td>
