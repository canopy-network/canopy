...
export const FieldControl: React.FC<Props> = ({
    ...
    const { data: dsValue, isLoading: dsLoading } = useFieldDs(f, templateContext)
    ...
    const isVisible = (f as any).showIf == null ? true : templateBool((f as any).showIf, templateContext)
    ...
    if (!FieldRenderer) {
        return (
            <div className="col-span-12 text-sm text-text-muted">Unsupported field type: {f.type}</div>
        )
    }
    ...
    return (
        <FieldRenderer
            ...
            setVal={(id: string, val: any) => setVal(id, val)}
        />
    )
}
