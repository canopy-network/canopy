...
export const Governance = () => {
    ...
    const { data: votingPowerData } = useVotingPower(selectedAccount?.address || '');
    ...
    const mockPolls: Poll[] = useMemo(() => {
        ...
        return activeProposals.slice(0, 2).map(p => ({
            id: p.hash,
            hash: p.hash,
            title: p.title,
    ...
    const handleVoteProposal = useCallback((proposalHash: string, vote: 'approve' | 'reject') => {
        console.log(`Voting ${vote} on proposal ${proposalHash}`);

        // Find the vote action in the manifest
        const voteAction = manifest?.actions?.find((action: any) => action.id === 'vote');
