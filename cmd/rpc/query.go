...
func (s *Server) IndexerBlobs(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	// extend this connection's write deadline alone to IndexerBlobsTimeoutS - a cold historical
	// read can legitimately run past the general RPC TimeoutS
...
func (s *Server) IndexerBlobsCached(ctx context.Context, height uint64) (result *fsm.IndexerBlobs, resultBz []byte, err lib.ErrorI) {
	// requestStart/path time this call end-to-end for every outcome; a failed call is recorded
	// separately (IndexerBlobErrorsTotal) rather than folded into the success-path metrics.
	...
	// deferred close records cancellation instead of polluting the cache-miss timing.
	cancelled := false
	defer func() {
		if cancelled {
	...
	// if set, else the general TimeoutS), for a warning that means what it says.
