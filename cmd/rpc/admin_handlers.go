...
// KeystoreGetKeyGroup queries the key group associated with an address or nickname
...
// ConsensusInfo queries node consensus information
...
// PeerInfo queries node peer information
...
// PeerBook queries the node's peer book
...
// Config queries the node's configuration file
func (s *Server) Config(w http.ResponseWriter, _ *http.Request, _ httprouter.Params) {
	write(w, s.config, http.StatusOK)
}

// txHandler is a helper function that abstracts the common workflow of sending transactions
...
func (s *Server) AddVote(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	// Create a proposal from the provided data bytes within the vote request.
	prop, err := types.NewProposalFromBytes(j.Proposal)
	// Check if there was an error in proposal creation or if the proposal has no end height.
	if err != nil || prop.GetEndHeight() == 0 {
		write(w, err, http.StatusBadRequest)
		return
	}
	// Attempt to add the proposal with the voting decision (approve/disapprove).
...
// ResourceUsage queries node resource usage
...
// keystoreHandler is a helper function that abstracts the common workflow of keystore operations
...
// getAddressFromNickname queries the account address for the supplied nickname
