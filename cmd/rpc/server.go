...
func TransactionLockOrder(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	txHandler(w, r, func(p crypto.PrivateKeyI, ptr *txRequest) (lib.TransactionI, error) {
		...
		return types.NewLockOrderTx(p, lib.LockOrder{OrderId: ptr.OrderId, LockerSendAddress: p.PublicKey().Address().Bytes(), LockerReceiveAddress: ptr.ReceiveAddress}, conf.NetworkID, conf.ChainId, app.ChainHeight(), ptr.Fee)
	})
}
