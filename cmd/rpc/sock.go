...
func (r *RCManager) GetHeight(rootChainId uint64) uint64 {
        // lock for thread safety: the subscriptions map and each subscription's Info are
        // mutated concurrently by the websocket read-loop and AddSubscription/RemoveSubscription
        r.l.Lock()
        defer r.l.Unlock()
...
func (r *RCManager) GetValidatorSet(rootChainId, id, rootHeight uint64) (lib.ValidatorSet, lib.ErrorI) {
        defer lib.TimeTrack(r.log, time.Now(), 500*time.Millisecond)
        // lock for thread safety while reading the subscription and its Info snapshot
        r.l.Lock()
        sub, found := r.subscriptions[rootChainId]
        if !found {
                r.l.Unlock()
        ...
        infoHeight, validatorSet, lastValidatorSet := sub.Info.Height, sub.Info.ValidatorSet, sub.Info.LastValidatorSet
        r.l.Unlock()
        // if rootHeight is the same as the RootChainInfo height
        if rootHeight == infoHeight || rootHeight == 0 {
                // exit with a copy the validator set
                return lib.NewValidatorSet(validatorSet)
        }
        // if rootHeight is 1 before the RootChainInfo height
        if rootHeight == infoHeight-1 {
                // exit with a copy of the previous validator set
                return lib.NewValidatorSet(lastValidatorSet)
        }
        // warn of the remote RPC call to the root chain API
        r.log.Warnf("Executing remote GetValidatorSet call with requested height=%d for rootChainId=%d with latest root height at %d", rootHeight, rootChainId, infoHeight)
...
func (r *RCManager) GetOrders(rootChainId, rootHeight, id uint64) (*lib.OrderBook, lib.ErrorI) {
        ...
        infoHeight, orders := sub.Info.Height, sub.Info.Orders
        r.l.Unlock()
        // if the root chain id and height is the same as the info
        if infoHeight == rootHeight {
                // exit with the order books from memory
                return orders, nil
        }
        // warn of the remote RPC call to the root chain API
        r.log.Warnf("Executing remote GetOrders call with requested height=%d for rootChainId=%d with latest root height at %d", rootHeight, rootChainId, infoHeight)
...
func (r *RCManager) GetOrder(rootChainId, height uint64, orderId string, chainId uint64) (*lib.SellOrder, lib.ErrorI) {
        defer lib.TimeTrack(r.log, time.Now(), 500*time.Millisecond)
        // lock for thread safety while reading the subscription
        r.l.Lock()
        sub, found := r.subscriptions[rootChainId]
        r.l.Unlock()
        if !found {
...
func (r *RCManager) GetLotteryWinner(rootChainId, height, id uint64) (*lib.LotteryWinner, lib.ErrorI) {
        ...
        infoHeight, lotteryWinner := sub.Info.Height, sub.Info.LotteryWinner
        r.l.Unlock()
        // if the root chain id and height is the same as the info
