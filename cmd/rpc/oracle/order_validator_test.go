...
// TestValidateOrderJsonBytes_InvalidOrderType verifies the default: branch of
// ValidateOrderJsonBytes is hit when the order type is neither LockOrderType nor
// CloseOrderType.
func TestValidateOrderJsonBytes_InvalidOrderType(t *testing.T) {
	tests := []struct {
		name      string
		orderType types.OrderType
	}{
		{
			name:      "unknown string order type",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := validator.ValidateOrderJsonBytes([]byte(`{}`), tt.orderType)
			if err == nil {
				t.Fatalf("ValidateOrderJsonBytes() expected error but got none")
			}
			if err.Error() != "error validating json bytes: Invalid order type" {
				t.Errorf("ValidateOrderJsonBytes() error = %q, want %q", err.Error(), "error validating json bytes: Invalid order type")
			}
		})
	}
}
