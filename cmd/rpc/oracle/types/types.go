...
// OracleDebugOrder is the oracle-side half of the combined order/oracle debug view: the locally
// witnessed lock/close order for a given order id, joined with the oracle's current safe/source
// chain height and confirmation config at query time
type OracleDebugOrder struct {
	...
	// ConfirmationLag is SourceChainHeight - SafeHeight
	ConfirmationLag uint64 `json:"confirmationLag"`
	// SafeBlockConfirmations is the oracle's configured confirmation depth
	SafeBlockConfirmations uint64 `json:"safeBlockConfirmations"`
	// ProposeDelayBlocks is the oracle's configured propose delay
	ProposeDelayBlocks uint64 `json:"proposeDelayBlocks"`
	// LockOrder is the oracle's locally witnessed lock order for this order id, if any
	LockOrder *WitnessedOrder `json:"lockOrder,omitempty"`
	// CloseOrder is the oracle's locally witnessed close order for this order id, if any
	CloseOrder *WitnessedOrder `json:"closeOrder,omitempty"`
}
