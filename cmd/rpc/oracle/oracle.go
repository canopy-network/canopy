...
// DebugOrder returns the oracle's locally witnessed lock/close orders for orderId, joined with the
// oracle's current safe/source chain height and confirmation config. This exists so troubleshooting a
// stuck order doesn't require cross-referencing the RPC order book, the oracle's on-disk order store,
// and the metrics endpoint by hand - the units mismatch that caused the deadline-reset bug (canopy
// height vs source-chain height) is only visible when both are read together.
func (o *Oracle) DebugOrder(orderId []byte) *types.OracleDebugOrder {
	if o == nil {
		return nil
	}
	safeHeight := o.state.GetSafeHeight()
	...
	info := &types.OracleDebugOrder{
		...
		SafeBlockConfirmations: o.config.SafeBlockConfirmations,
		ProposeDelayBlocks:     o.config.ProposeDelayBlocks,
	}
	if lockOrder, err := o.orderStore.ReadOrder(orderId, types.LockOrderType); err == nil {
		info.LockOrder = lockOrder
	}
	if closeOrder, err := o.orderStore.ReadOrder(orderId, types.CloseOrderType); err == nil {
		info.CloseOrder = closeOrder
	}
	return info
}
