...
func NewEthBlockProvider(config lib.EthBlockProviderConfig, logger lib.LoggerI) *EthBlockProvider {
...
// fetchAndPrepareBlock fetches the block at the specified height and creates wrappers
func (p *EthBlockProvider) fetchAndPrepareBlock(ctx context.Context, height *big.Int) (*Block, error) {
	...
	// iterate through ethereum transactions, creating a Transaction wrapper and determing from address
	for _, ethTx := range ethBlock.Transactions() {
		...
		from, err := ethtypes.Sender(ethtypes.LatestSignerForChainID(big.NewInt(int64(p.chainId))), ethTx)
		if err != nil {
			return nil, err
		}
		// set from address
...
// monitorHeaders creates a new subcription for block headers
func (p *EthBlockProvider) monitorHeaders(ctx context.Context) error {
	...
	for {
		...
		case header := <-headerCh:
			...
			p.processSafeBlocks(ctx, header.Number)
		case err := <-sub.Err():
			sub.Unsubscribe()
			return err
		}
...
func (p *EthBlockProvider) processSafeBlocks(ctx context.Context, latestHeight *big.Int) {
	p.heightMu.Lock()
	defer p.heightMu.Unlock()
	// calculate safe height with confirmations
	safeHeight := new(big.Int).Sub(latestHeight, p.safeBlockConfirmations)
	...
	p.logger.Debugf("eth block provider processing safe blocks up to height: %d (current: %d)", safeHeight, latestHeight)
	// use safe height if next height is 0
	if p.nextHeight.Cmp(big.NewInt(0)) == 0 {
		p.nextHeight = new(big.Int).Set(safeHeight)
		p.logger.Infof("eth block provider initialized next height to safe height: %s", safeHeight.String())
	}
	...
	for p.nextHeight.Cmp(safeHeight) <= 0 {
		...
		block, err := p.fetchAndPrepareBlock(ctx, p.nextHeight)
		if err != nil {
