...
### 1. Transaction Processing: Order Detection and Validation

The system performs comprehensive transaction analysis for each block:

- **Order Parsing**: Examines transaction data to detect embedded Canopy orders using specific parsing logic
- **Success Validation**: Fetches transaction receipts to verify that transactions completed successfully, preventing processing of failed transactions
- **ERC20 Processing**: For ERC20 transactions, retrieves token metadata and validates contract interactions
- **Data Enrichment**: Populates transaction objects with sender addresses, token information, and order details

This process is similar to how payment processors validate transactions - they must verify both the transaction format and successful execution before processing.
