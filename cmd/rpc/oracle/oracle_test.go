...
func createSellOrder(orderIdHex string, data string, buyerReceiveAddress ...string) *lib.SellOrder {
	...
	sellOrder := &lib.SellOrder{
		Id:              orderIdBytes,
		Committee:       1,          // Default committee for tests
		RequestedAmount: 100000000,  // Default amount for tests (100 tokens)
	}
	if data != "" {
...
// createSellOrderWithData creates a sell order with data field (contract address) and seller receive address
func createSellOrderWithData(orderIdHex string, data string, buyerReceiveAddress string, sellerReceiveAddress string) *lib.SellOrder {
	orderIdBytes := []byte(orderIdHex)
	sellOrder := &lib.SellOrder{
		Id:                   orderIdBytes,
		Committee:            1,          // Default committee for tests
...
func createWitnessedLockOrder(id string, buyerAddress ...string) *types.WitnessedOrder {
	...
	return &types.WitnessedOrder{
		...
		LockOrder: &lib.LockOrder{
			OrderId:             []byte(id),
			ChainId:             1, // Default committee for tests
...
func TestOracle_processBlock(t *testing.T) {
	...
	transactionWithOrder := func(order *types.WitnessedOrder, toAddress ...string) *mockTransaction {
		t := &mockTransaction{
			tokenTransfer: types.TokenTransfer{
				TokenBaseAmount: big.NewInt(100000000), // 100 token units for tests
			},
			order: order,
		}
		if len(toAddress) > 0 {
		...
		},
		{
			...
			block: createMockBlockWithTransactions(
				...
				[]types.TransactionI{
					...
					transactionWithOrder(createWitnessedCloseOrder("order1", 1), contractAddress),
				},
			),
			orderStore: createOrderStore(),
		...
		},
		{
			...
			block: createMockBlockWithTransactions(
				...
				[]types.TransactionI{
					...
					transactionWithOrder(createWitnessedCloseOrder("close1", 1), contractAddress),
					&mockTransaction{},
				},
			),
			orderStore: createOrderStore(),
	...
	tests := []struct {
		...
		},
		{
			...
			transactions: []types.TransactionI{
				&mockTransaction{
					...
					tokenTransfer: types.TokenTransfer{
						TokenBaseAmount: big.NewInt(100000000), // 100 USDC
					},
				},
				&mockTransaction{
					to:    usdtContract,
		...
		},
		{
			...
			orderBook: createOrderBook(
				...
				createSellOrderWithData(usdtOrderId, usdtContract, buyerReceiveAddress, sellerReceiveAddress),
			),
			witnessedOrders: []*types.WitnessedOrder{
				createWitnessedCloseOrder(usdtOrderId, 1),
			},
