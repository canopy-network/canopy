...
| Path | Returns |
|------|---------|
| `/v1/health` | `{height, genesisComplete, chainId, tvlCapUcnpy, tvlUtilizationBps}` |
...
`tvlCapUcnpy` is the deposit ceiling (`0` = uncapped) and `tvlUtilizationBps` is how full it
is in basis points. See [TVL Cap](../advanced/tvl-cap.mdx).
