...
| Path | Returns |
|------|---------|
| `/v1/health` | `{height, genesisComplete, chainId, tvlCapBps, canopyTotalStake, tvlCapUcnpyEffective, tvlUtilizationBps}` |
| `/v1/globals` | `CanoliqGlobals` (singleton accounting record) |
| `/v1/params` | `CanoliqParams` (governance-tunable knobs) |
| `/v1/pools` | Committee pool, treasury (CNPY/CLIQ), buyback, insurance, per-validator incentives, peak TVL, insurance target & funded-bps |
...
| `/v1/stakers` | Active CLIQ stakers with amount and `stakedAtHeight` |
...
33%; `0` = uncapped). `tvlCapUcnpyEffective` is the live computed uCNPY cap
(`mulDiv(canopyTotalStake, tvlCapBps, 10_000)`; `0` when uncapped or Canopy stake is
unavailable). `tvlUtilizationBps` is `totalPooledCnpy / tvlCapUcnpyEffective` in basis
points. See [TVL Cap](../advanced/tvl-cap.mdx).
...
> read `0` until the first MessageStake lands on Canopy.
