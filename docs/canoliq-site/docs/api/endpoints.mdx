...
**In one sentence:** a read-only HTTP API for monitoring a running node — no transaction can be
...
`"awaiting-canopy-stake"`, `"fail-closed"` — operators rely on this to tell apart the two
rejecting states when `tvlCapUcnpyEffective` is `0`: `"fail-closed"` is a misconfiguration
someone must act on, `"awaiting-canopy-stake"` resolves itself once Canopy has bonded stake.
Only `"uncapped"` accepts deposits with `tvlCapUcnpyEffective` at `0`.
...
GET /v1/pools
{
  ...
  "otcBudgetAvailable": "500000000000",
  "otcBudgetReserved": "2500000000"
}
```
...
`otcBudgetAvailable` is unreserved [OTC lock program](../tokenomics/otc-lock-program.mdx) CPLQ;
`otcBudgetReserved` is the total already promised to open lock positions. Both are in uCPLQ.
...
GET /v1/account/{addr}
{
  ...
  "otcLocks": [
    {
      "id": 1,
      "ccnpyAmount": "50000000000",
      "tier": "OTC_LOCK_90D",
      "rewardCplq": "2500000000",
      "startHeight": 120000,
      "matureHeight": 1416000
    }
  ]
}
```

`otcLocks` lists the caller's open [OTC lock positions](../tokenomics/otc-lock-program.mdx) and
is omitted entirely when there are none. `matureHeight` is the first block at which the position
can be claimed.

## Known Gaps

There is no dedicated route for OTC lock positions. They are returned on `/v1/account/{addr}`
alongside vesting schedules, which covers the per-address case; listing every open position
across all addresses is not currently possible over HTTP.
