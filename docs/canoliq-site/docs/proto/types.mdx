...
message CanoliqParams {
  ...
  uint64 cliq_unstaking_blocks = 18;     // Default 100_800 (~7d); must be ≥ voting period
  uint64 proposal_fee = 19;
  uint64 vote_fee = 20;
  uint64 stake_fee = 21;
  uint64 multisig_approve_fee = 22;
  uint64 min_stake_to_propose = 23;      // Default 1_000_000 (1 CLIQ)
...
message GovernanceTier {
  ActionType action = 1;
  uint64 quorum_bps = 2;            // fraction of snapshot staked CLIQ that must vote
...
message UnstakingIndex {
  repeated uint64 ids = 1;  // pending CLIQ unstake ids per address
}
```
...
message CLIQStakeIndex {
  repeated bytes addresses = 1;  // addresses with non-zero CLIQStake.amount
}
```
