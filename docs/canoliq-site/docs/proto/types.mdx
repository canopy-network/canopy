...
**In one sentence:** the shapes of everything canoLiq keeps in state — globals, params,
stakes, proposals, and the rest — as stored, not as sent in transactions.
...
message CanoliqGlobals {
  ...
  uint64 next_otc_lock_id = 22;          // monotonic OTC lock position id counter
}
```
...
message CanoliqParams {
  ...
  // T3 — TVL deposit cap, as a fraction of total Canopy stake
  uint64 tvl_cap_bps = 25;               // Default 3300 (33% of Canopy stake); 0 = uncapped
  ...
  // OTC lock program — reward rates and minimum position size
  uint64 otc_tier90_bps = 33;            // Default 500 (5% of locked cCNPY)
  uint64 otc_tier120_bps = 34;           // Default 800 (8% of locked cCNPY)
  uint64 otc_min_lock_uccnpy = 35;       // Default 50_000_000_000 (50,000 cCNPY)
  uint64 otc_tier90_blocks = 36;         // Default 1_296_000 (~90d at 6s blocks)
  uint64 otc_tier120_blocks = 37;        // Default 1_728_000 (~120d at 6s blocks)
}
```
...
An open [OTC lock](../tokenomics/otc-lock-program.mdx) position. The locked cCNPY is held here
rather than on the holder's spendable balance, which is what makes it unredeemable for the term.
`reward_cplq` is fixed at creation and reserved from the program budget at that moment.

```protobuf
message OTCLock {
  uint64 id = 1;
  bytes address = 2;
  uint64 ccnpy_amount = 3;     // uccnpy held by this position
  OTCLockTier tier = 4;
  uint64 reward_cplq = 5;      // uCPLQ reserved against this position
  uint64 start_height = 6;
  uint64 mature_height = 7;    // first height at which a claim succeeds
}
```
...
message OTCLockIndex {
  repeated uint64 ids = 1;  // open OTC lock position ids per address
}
```
...
enum ActionType {
  ...
  ACTION_OTC_PROGRAM_FUND = 8;   // 10% quorum / 67% approval / no timelock
}
```
...
Identifies which [OTC lock](../tokenomics/otc-lock-program.mdx) tier a position belongs to.
Unrelated to `LockTier`: this locks cCNPY for a one-off CPLQ reward, not CPLQ for voting weight.

The names are fixed identifiers, not a restatement of the terms — each tier's actual length comes
from its `otc_tier*_blocks` parameter and is governance-tunable.

```protobuf
enum OTCLockTier {
  OTC_LOCK_UNSPECIFIED = 0;  // rejected; never a valid choice
  OTC_LOCK_90D = 1;          // term otc_tier90_blocks;  pays otc_tier90_bps
  OTC_LOCK_120D = 2;         // term otc_tier120_blocks; pays otc_tier120_bps
}
```
