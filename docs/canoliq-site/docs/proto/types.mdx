...
enum ActionType {
  ...
  ACTION_OTC_PROGRAM_FUND = 8;   // no configured tier — uses the scalar fallback
}
```
...
Term length for an [OTC lock](../tokenomics/otc-lock-program.mdx) position. Unrelated to
`LockTier`: this locks cCNPY for a one-off CPLQ reward, not CPLQ for voting weight.

```protobuf
enum OTCLockTier {
  OTC_LOCK_UNSPECIFIED = 0;  // rejected; never a valid choice
  OTC_LOCK_90D = 1;          // 1,296,000 blocks; pays otc_tier90_bps
  OTC_LOCK_120D = 2;         // 1,728,000 blocks; pays otc_tier120_bps
}
```
