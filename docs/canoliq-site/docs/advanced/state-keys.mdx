...
| Treasury CLIQ | 11 | Pool scalar | 2 |
| CLIQ Stake | 12 | Per-address | 2 |
| CLIQ Unstaking | 13 | Per-address, per-id | 2 |
...
:::note
There is no range-scan. The FSM only answers point reads, so iteration requires an explicit index. `VestingIndex`, `ProposalIndex`, `CLIQStakeIndex`, and the spend index all exist for this reason.
:::
