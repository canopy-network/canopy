...
| `MessageCanoliqTransfer` | Transfers liquid cCNPY between accounts (pure balance move, no mint/burn) | 1 |
...
| `MessageCPLQProposalCreate` | Opens a governance proposal (one of [seven payload types](../governance/proposals.mdx)) | 2 |
...
| `MessageOTCLockCreate` | Locks cCNPY for a 90d/120d term; reserves its CPLQ reward | 3 |
| `MessageOTCLockClaim` | Releases a matured lock: cCNPY back plus the reserved CPLQ | 3 |
| `MessageOTCLockCancel` | Early exit: cCNPY back, CPLQ reward forfeited to the budget | 3 |
...
| cCNPY Transfer | `canoliq_transfer_fee` |
