...
func (s *StateMachine) PollsToResults(polls *ActivePolls) (result Poll, err lib.ErrorI) {
        ...
        for proposalHash, addresses := range polls.Polls {
                ...
                // calculate stats for validators, guarding against division by zero when there's no validator power to weigh votes against
                if r.Validators.TotalTokens != 0 {
                ...
                // calculate stats for accounts, guarding against division by zero when there's no token supply to weigh votes against
                if r.Accounts.TotalTokens != 0 {
